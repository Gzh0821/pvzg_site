...
Панель GP-Next сейчас в основном включает:

- **Patcher**: управление `packs/`, `patches/` и ручными правками; открытие папки данных; сохранение порядка; перезагрузка патчей.
- **Data**: просмотр данных игры в реальном времени, экспорт, сравнение, ручное редактирование и восстановление отдельной записи или целого типа.
- **Trainer**: чит-функции для сцен боя, карты мира и сценариев, связанных с sandbox.
- **Settings**: язык, frame rate, debug, оптимизация прокрутки, Runtime Extensions и переключатели HP Overlay.
- **Guide / About / Log**: встроенная документация, справка по командам/API и просмотрщик runtime-логов.
...
com.pvzge.app/
└── gp-next/
    ├── settings.json        ← Локальные настройки GP-Next: порядок / отключенное состояние паков
    ├── packs/
    ...
    └── patches/            ← Одиночные патчи
        └── jsons/
            ...
            ├── objects/
            └── levels/
    └── __gpn_edits/        ← Сохраненные ручные правки из вкладки Data (максимальный приоритет)
```
...
- **`settings.json`**: локальный файл настроек GP-Next, используемый для сохранения порядка datapack, отключенного состояния и связанных настроек панели.
- **`__gpn_edits/`**: хранит ручные изменения, сделанные через вкладку **Data**. Эти правки всегда перекрывают обычные datapack и одиночные патчи.
...
## Настройки, Runtime Extensions и вспомогательные функции

Помимо загрузки патчей, GP-Next также включает несколько runtime-возможностей:

- **Scrolling Optimization**: позволяет отдельно настраивать масштабирование непрерывной прокрутки wheel/scroll и минимальный интервал для дискретного переключения селекторов. Это влияет на настройки, экраны типа альманаха, world chooser, sandbox-селекторы растений/зомби и некоторые wheel-переключатели в sandbox.
- **Runtime Extensions**: сейчас в первую очередь включает `Dynamic Plant Registry`, который дает datapack с добавлением или клонированием растений более надежное сопоставление идентичности в runtime. Обычно достаточно перезагрузить патчи; полный перезапуск игры не обязателен.
- **HP Overlay**: может показывать HP растений, зомби и наземных объектов типа Tomb в реальном времени во время боя. Для сущностей с дополнительным shell/armor-слоем может также отображаться дополнительный слой урона.
- **Update Check и Logs**: в футере показываются версия и статус обновлений, а вкладка **Log** дает доступ к runtime-логам GP-Next для диагностики.
