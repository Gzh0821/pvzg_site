...
The `aliases` array contains the plant's `CODENAME`, used to indicate the corresponding plant for this item. Only the first item is read from at the moment. The value of `objclass` is `PlantAlmanacProperties`, indicating that this item is a plant almanac property.
...
The `aliases` array contains the plant's `CODENAME`, used to indicate the corresponding plant for this item. Again, only the first entry is read. The value of `objclass` is `PlantProperties`, indicating that this item is a plant numerical property.
